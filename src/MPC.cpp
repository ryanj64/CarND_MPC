...
Eigen::MatrixXd MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  bool ok = true;
  ...
  Eigen::MatrixXd vxd(8, N);

  for(unsigned int t = 0; t < N; t++)
  {
    vxd(0, t) = solution.x[x_start + t];
    vxd(1, t) = solution.x[y_start + t];
    vxd(2, t) = solution.x[psi_start + t];
    vxd(3, t) = solution.x[v_start + t];
    vxd(4, t) = solution.x[cte_start + t];
    vxd(5, t) = solution.x[epsi_start + t];
  }

  for(unsigned int t = 0; t < N-1; t++)
  {
    vxd(6, t) = solution.x[delta_start + t];
    vxd(7, t) = solution.x[a_start + t];
  }
